...
class PayrecordAction extends SystemAction{
	
	public function index(){
		import('@.ORG.Page');
		$wxuser_db = D('Inrecords');
       if (IS_POST) {
        ...
        $count     = $wxuser_db->where(array('uid' => session('dluserId')))->count();
        $Page      = new Page($count, 15);
        $show      = $Page->show();
        $list      = $wxuser_db->where(array('uid' => session('dluserId')))->limit($Page->firstRow . ',' . $Page->listRows)->select();
