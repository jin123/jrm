...
class AlipayAction extends Action{
	...
	public function __construct(){
		...
		$this->alipayConfig=$alipay_config_db->find();
	}
	public function pay(){
		//参数数据
		$price=$_POST['price'];
		$orderName=$_POST['uname'];
		$orderid=time();
		//if (!$orderid){
		...
		//必填，不能修改
		//服务器异步通知页面路径
		$notify_url = C('site_url').'/index.php?g=Daili&m=Alipay&a=notify_url';
		//需http://格式的完整路径，不能加?id=123这类自定义参数
		//页面跳转同步通知页面路径
		$return_url = C('site_url').'/index.php?g=Daili&m=Alipay&a=return_url';
			...
			if( $this->_get('trade_status') == 'TRADE_FINISHED' ||  $this->_get('trade_status') == 'TRADE_SUCCESS') {
				...
				if($order){
					...
					//if($back!=false&&$add!=false){
						$this->redirect('Product/my',array('token'=>$order['token'],'wecha_id'=>$order['wecha_id'],'success'=>1));
					//}else{
