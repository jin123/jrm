<?php
class Member_cardAction extends CommonAction
{
        ...
        if (IS_POST) {
            ...
            if ($data == false) {
                $data = array(
                    ...
                    'bg' => '/Public/images/card/card_bg15.png',
                    'diybg' => '/Public/images/card/card_bg17.png',
                    'msg' => '微会员卡，方便携带收藏，永不挂失',
                    'numbercolor' => '#000000',
                    'vipnamecolor' => '#121212',
                    'Lastmsg' => '/Public/images/cart_info/news.jpg',
                    'vip' => '/Public/images/cart_info/vippower.jpg',
                    'qiandao' => '/Public/images/cart_info/qiandao.jpg',
                    'shopping' => '/Public/images/cart_info/shopping.jpg',
                    'memberinfo' => '/Public/images/cart_info/user.jpg',
                    'membermsg' => '/Public/images/cart_info/vippower.jpg',
                    'contact' => '/Public/images/cart_info/addr.jpg'
                );
            }
    ...
    public function create()
    {
        ...
        if (IS_POST) {
			...
			$this->assign('id',$this->thisCard['id']);
            $this->display();
        }
    }
    public function create_add()
    {
        ...
        if (IS_POST) {
           ...
           /*if ($end == false || $stat == false) {
            ...
            if ($back != false) {
                $this->success('恭喜您共开了' . $j . '张会员卡', U('Member_card/create', array('id' => $this->thisCard['id'])));
            } else {
                $this->error('没开成任何会员卡');
            }
        } else {
    ...
    public function exchange()
    {
        ...
        if (IS_POST) {
            ...
            if ($data == false) {
                $this->all_insert('Member_card_exchange', '/exchange?id=' . $this->thisCard['id']);
            } else {
                $_POST['id'] = $data['id'];
                $this->all_save('Member_card_exchange', '/exchange?&id=' . $this->thisCard['id']);
            }
        } else {
    ...
    function noticeSet()
    {
        ...
        if (IS_POST) {
            ...
            $this->success('设置成功', U('Member_card/notice', array('id' => $this->thisCard['id'])));
        } else {
            if (isset($_GET['noticeid'])) {
    ...
    function noticeDelete()
    {
        ...
        $this->success('操作成功', U('Member_card/notice', array('id' => $this->thisCard['id'])));
    }
    ...
    public function coupon_edit()
    {
        ...
        if (IS_POST) {
			...
			$_POST['statdate'] = mktime(23, 59, 59, $statdate[1], $statdate[2], $statdate[0]);
			$_POST['enddate'] = mktime(23, 59, 59, $enddate[1], $enddate[2], $enddate[0]);

            if (!isset($_GET['itemid'])) {
                $this->all_insert('Member_card_coupon', '/coupon?id=' . $this->thisCard['id']);
            } else {
				$member_card_coupon_db->where(array('id' => intval($_GET['itemid'])))->save($_POST);
    ...
    public function integral_edit()
    {
        ...
        if (IS_POST) {
            ...
            $_POST['enddate'] = mktime(23, 59, 59, $enddate[1], $enddate[2], $enddate[0]);
            if (isset($_GET['itemid'])) {
                $member_card_inergral_db->where(array(
                    'id' => intval($_GET['itemid'])
                ))->save($_POST);
    ...
    public function info()
    {
        ...
        if (IS_POST) {
			...
			$_POST['cardid']  = $this->thisCard['id'];
            if ($data == false) {
                $this->all_insert('Member_card_info', '/index');
            } else {
                $_POST['id'] = $data['id'];
                $this->all_save('Member_card_info', '/index');
            }
        } else {
