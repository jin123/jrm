<?php
class PanoramaAction extends CommonAction{
	...
	public function add(){ 
		if(IS_POST){
			...
			$set=array();
			$baseUrl='/public/images/panorama/';
	...
	public function set(){
		...
		if(empty($checkdata)||$checkdata['token']!=session('token')){
		...
		if(IS_POST){ 
			...
			if($this->panorama_model->create()){
				if($this->panorama_model->where($where)->save($_POST)){
					$this->success('修改成功',U('Panorama/index'));
					$keyword_model=M('Keyword');
					$keyword_model->where(array('token'=>session('token'),'pid'=>$id,'module'=>'Panorama'))->save(array('keyword'=>$_POST['keyword']));
				}else{
					$this->error('操作失败');
				}
			}else{
	...
	public function delete(){
        ...
        if(IS_GET){                              
            ...
            if($back==true){
            	$keyword_model=M('Keyword');
            	$keyword_model->where(array('token'=>session('token'),'pid'=>$id,'module'=>'Panorama'))->delete();
                $this->success('操作成功',U('Panorama/index'));
            }else{
                 $this->error('服务器繁忙,请稍后再试',U('Panorama/index'));
            }
        }        
	}
}
