...
/*		$agent = $_SERVER['HTTP_USER_AGENT']; 
		...
		$where 		= array('wecha_id'=>$wecha_id,'lid'=>$id);
		...
		$data['lid']		= $record['lid'];
		$data['rid']		= $record['id'];
		$data['usenums'] 	= $record['wecha_name'];
	...
	public function getajax(){	
		...
		if ($record['islottery'] == 1) {				
			...
			$prize	 = $record['prize'];
			$tel 	 = $record['phone'];
			$msg = "您已经中过<font color='red'> $prize</font> 了,您的领奖序列号:<font color='red'> $sn </font>请您牢记及尽快与我们联系.";//尊敬的：<font color='red'>$uname</font>,
	...
	public function add(){
		 if($_POST['action'] ==  'add' ){
			...
			echo'{"success":1,"msg":"恭喜中奖了！请您保持手机通畅！你的领奖序号：'.$data['sn'].'"}';
			exit;
		}
	}
	...
	public function adduser(){
		 if($_POST['action'] ==  'adduser' ){
			...
			echo'{"success":1,"msg":"填写用户资料成功！请您现在抽奖"}';
			exit;
		}
	}
